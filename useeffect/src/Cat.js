...
const Cat = () => {
  ...
  // change color every 5 seconds based on prev num
  useEffect(() => {
    const colorInterval = setInterval(() => {
      // % colors.length causes it to loop
  ...
  // set local storage cat status whenever status is changed
  useEffect(() => {
    localStorage.setItem("catStatus", statusChange);

    // no cleanup, so the stored variable is saved
  }, [statusChange]);

  // for statusChange edge cases
  useEffect(() => {
    // if status empty
    if (statusChange === "") {
